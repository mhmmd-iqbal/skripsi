...
class ModelPenjualan extends Model
{
    ...
    function get_datatables($year = null)
    {
        ...
        if ($year != null) {
            $db->where('tb_penjualan.tahun', $year);
        }
    ...
    {
        $db    = \Config\Database::connect()->table($this->table)->join('tb_desa', 'tb_desa.id = tb_penjualan.id_desa')->join('tb_kecamatan', 'tb_kecamatan.id = tb_desa.id_kecamatan');
        if ($year != null) {
            $db->where('tb_penjualan.tahun', $year);
        }
        if (($_POST['kecamatan'] != null) && ($_POST['kecamatan'] != '')) {
